...
object Mqtt {
	...
	def main(args: Array[String]): Unit = {
		...
		val deviceId = "eb71d42c-772e-4de0-8e1a-0dfcbf3c1025"
		val key = "readingNumber"
		val start = new DateTime(2019, 1, 10, 1, 0)
		val end = new DateTime(2019, 1, 18, 1, 0)
		val stepHour = 2
		val startVal: Double = 94
		val stepVal: Double = 0.04
		val randVal: Double = 0.01
		val precision = 2
		...
		while (time.isBefore(end)) {
			if (time.getHourOfDay <= skipBeginHour && time.getHourOfDay >= skipEndHour) {
				val data = IotaData("1b2d8739-627e-4b7d-9480-3eee6e9396fe",
					...
					time, time,
					IotaAcqResult(ErrorInfo(0, None, None), Some(Map(key -> value)))
				)

				time = time.plusHours(stepHour)
				val tmp = value + stepVal + (Random.nextDouble() - 0.5) * randVal
				value = BigDecimal(tmp).setScale(precision, RoundingMode.HALF_UP).toDouble
