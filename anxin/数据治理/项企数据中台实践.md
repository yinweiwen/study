...
猜想：pom中引用的hadoop-common其实无效，已被其他的flink相关包中的覆盖。而打包发布的时候，flink相关包是provide范围的。
...
问题说明：在独立部署的flink-session中启动没有报错，在k8s部署的flink环境中出错

查看独立部署的flink的lib，发下有两个hadoop相关的库：
